...
const animStyle = `
  ...
  .table-row { animation: fadeIn 0.2s ease; }
        ...
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && closeModal()}
          ...
          <div className="modal-box" style={{ background: "#fff", borderRadius: "14px", width: "100%", maxWidth: "540px", maxHeight: "90vh", overflowY: "auto", padding: "28px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              <Field label="Nome completo" required><input className="input-field" type="text" placeholder="Nome do paciente" value={form.name} onChange={(e) => handleField("name", e.target.value)} style={inputStyle} /></Field>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                <Field label="Telefone" required><input className="input-field" type="text" placeholder="(00) 00000-0000" value={form.phone} onChange={(e) => handleField("phone", maskPhone(e.target.value))} style={inputStyle} /></Field>
                <Field label="CPF"><input className="input-field" type="text" placeholder="000.000.000-00" value={form.cpf} onChange={(e) => handleField("cpf", maskCPF(e.target.value))} style={inputStyle} /></Field>
                <Field label="Email"><input className="input-field" type="email" placeholder="email@exemplo.com" value={form.email} onChange={(e) => handleField("email", e.target.value)} style={inputStyle} /></Field>
                <Field label="Data de nascimento"><input className="input-field" type="date" value={form.birth} onChange={(e) => handleField("birth", e.target.value)} style={inputStyle} /></Field>
              </div>
              <Field label="Endereço"><input className="input-field" type="text" placeholder="Rua, número, bairro..." value={form.address} onChange={(e) => handleField("address", e.target.value)} style={inputStyle} /></Field>
              <Field label="Alergias"><input className="input-field" type="text" placeholder="Ex: Latex, Anestésicos..." value={form.allergies} onChange={(e) => handleField("allergies", e.target.value)} style={inputStyle} /></Field>
              <Field label="Histórico médico"><textarea className="input-field" placeholder="Condições relevantes..." value={form.history} onChange={(e) => handleField("history", e.target.value)} style={{ ...inputStyle, minHeight: "80px", resize: "vertical" }} /></Field>
              <Field label="Observações"><textarea className="input-field" placeholder="Observações adicionais..." value={form.obs} onChange={(e) => handleField("obs", e.target.value)} style={{ ...inputStyle, minHeight: "80px", resize: "vertical" }} /></Field>
            </div>
