...
export default function Dashboard() {
  ...
  return isDentista ? <DashboardDentista /> : <DashboardPaciente auth={auth} />;
}
    ...
    <div style={{ display: "flex", background: "#f0f4f8", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, padding: "28px 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px", marginBottom: "24px" }}>
          <Card label="Consultas Hoje" value={todayAppointments.length} sub={`${todayAppointments.length} concluídas`} iconBg="#e3f2fd" iconColor="#1976d2" icon={<CalIcon />} />
          <Card label="Total de Pacientes" value={patients.length} sub="Cadastrados" iconBg="#e8f5e9" iconColor="#388e3c" icon={<UserIcon />} />
          <Card label="Receita do Mês" value={receitaMes.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })} sub="Pagamentos confirmados" iconBg="#fff8e1" iconColor="#f59e0b" icon={<MoneyIcon />} />
          <Card label="Taxa de Presença" value={`${taxaPresenca}%`} sub="Nos últimos registros" iconBg="#fce4ec" iconColor="#e91e63" icon={<TrendIcon />} />
        </div>
...
function DashboardPaciente({ auth }) {
  ...
  const today = new Date().toISOString().split("T")[0];
  const minhasConsultas = appointments.filter((a) => a.patient === auth?.user);
  ...
  return (
    <div style={{ display: "flex", background: "#f0f4f8", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, padding: "28px 32px" }}>
        <div style={{ marginBottom: "24px" }}>
          <h1 style={{ fontSize: "24px", fontWeight: 700, color: "#1a1a2e", margin: 0 }}>Olá, {auth?.user}! 👋</h1>
          <p style={{ margin: "4px 0 0", fontSize: "13px", color: "#888" }}>Bem-vindo à sua área de paciente</p>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "24px" }}>
          <Card label="Consultas Hoje" value={consultasHoje.length} sub="Agendadas para hoje" iconBg="#e3f2fd" iconColor="#1976d2" icon={<CalIcon />} />
          <Card label="Próximas Consultas" value={proximasConsultas.length} sub="Agendadas" iconBg="#e8f5e9" iconColor="#388e3c" icon={<UserIcon />} />
          <Card label="Total de Consultas" value={minhasConsultas.length} sub="No histórico" iconBg="#fff8e1" iconColor="#f59e0b" icon={<TrendIcon />} />
        </div>
        ...
        <div style={cardStyle}>
          ...
          {proximasConsultas.length === 0 ? (
            ...
            proximasConsultas.map((a) => (
              <div key={a.id} style={listItem}>
                <div style={{ width: 44, height: 44, borderRadius: "10px", background: "#dbeafe", color: "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "14px", flexShrink: 0 }}>
                  {a.time}
                </div>
                <div>
...
function Card({ label, value, sub, iconBg, iconColor, icon }) {
  return (
    <div className="summary-card" style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8edf5", padding: "20px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
      ...
      <div style={{ width: 44, height: 44, borderRadius: "10px", background: iconBg, color: iconColor, display: "flex", alignItems: "center", justifyContent: "center" }}>{icon}</div>
    </div>
  );
}
